...
/**
 * The LeafNode class represents a leaf node of the DNATree data structure. This type of leaf node stores a DNA Sequence.
 *
...
public class LeafNode extends DNATreeNode {
    ...
    /**
     * Finds the correct direction based on sequence order for the a leaf node being traversed.
     * 
     * 
     * @param depth
    ...
    public int getIndexOfByDepth(int depth) {
        ...
        if (depth < dnaSequence.length) {
        	int index = DNASequence.indexOf(dnaSequence[depth]);
            return index; 
        }
        return 4;
    ...
    public void printNodeStats(int depth) {
    	...
    	System.out.println(String.valueOf(dnaSequence) + " A:" + statsA + " C:" + statsC + " G:" + statsG + " T:" + statsT);
    }
    ...
    /**
     * Method implemented by LeafNode and EmptyNode and overridden by
     * InternalNode
     * for recursively finding a given sequence(s) and storing it in results,
     * based
     ...
     *            the search term
     * @param match
     *            true if search term is a match
     * @param res
