...
/**
 * The InternalNode class represents an internal node of the DNAtree data structure. This type of node is defined as a node with 5 children of type FlyweightNode.
 *
...
public class InternalNode extends DNATreeNode {
    ...
    /**
     * Constructor that instantiates the children nodes with an instance of FlyweightNode
     */
    public InternalNode() {
    ...
    /**
     * Insert method that implements the insertion of a LeafNode instance. The FlyweightNode instance is replaced with LeadNode since a LeafNode our only node that can hold data in itself.
     *
     * @param depth
     ...
     * @param isNewNode
     *            is false when newLeafNode has to be reorganized within the DNAtree. Is true when newLeafNode is a new node for the DNAtree.
     */
    public void insert(int depth, LeafNode newLeafNode, boolean isNewNode) {
        ...
        if (child instanceof LeafNode) {
            if (position == 4) {
                System.out.println("sequence " + newLeafNode + " already exists");
                return;
            }
...
// }
    ...
    public DNATreeNode remove(int depth, LeafNode nodeToRemove) {
        ...
        DNATreeNode nodeValue = internaleafNode.remove(depth + 1, nodeToRemove);
