...
// }
    ...
    public void testLeadingZeroesOnZero() {
        ...
        assertEquals(rpn.calculateLine(operationLine1), "0 255 + = 255");
        assertEquals(rpn.calculateLine(operationLine2), "40 255 + = 295");
        assertEquals(rpn.calculateLine(operationLine3), "4 255 + = 259");
        assertEquals(rpn.calculateLine(operationLine4), " 255 + = 255");
        assertEquals(rpn.calculateLine(operationLine5), "5 255 / =");
        
    }
    
    public void testSumAndMultiply() {
        String[] operationLine1 = {"000000", "255", "+", "45", "*"};
        assertEquals(rpn.calculateLine(operationLine1), "0 255 + 45 * = 11475");
    }
    
    public void testSumAndExponent() {
        String[] operationLine1 = {"8", "3", "^", "10", "+"};
        assertEquals(rpn.calculateLine(operationLine1), "8 3 ^ 10 + = 522");
    }
    
    public void testMultiplyAndExponent() {
        String[] operationLine1 = {"10", "5", "*", "2", "^"};
        assertEquals(rpn.calculateLine(operationLine1), "10 5 * 2 ^ = 2500");
    }
    
    
    public void testScanFileValid() {
        String inputFile = "MixedOperations.txt";
        rpn.scanFile(inputFile);
        assertFuzzyEquals(systemOut().getHistory(), "40 255 + = 295\n20 30 * = 600");
    }
    
    
    public void testScanFileInvalid () {
        
        String inputFileNoSuch = "MixedOperationsFail.txt";
        Exception thrown = null;
        
        try {
            
            rpn.scanFile(inputFileNoSuch);
        }
        ...
        assert(thrown instanceof FileNotFoundException);
